    ...
    return (
        <section className="max-w-screen-2xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 px-4">
            {products.map(product => (
                <div key={product.id} className='bg-white border cursor-pointer hover:shadow-xl border-gray-200 rounded-lg overflow-hidden shadow-sm flex flex-col'>
                    <span className='text-xs text-gray-500 px-3 py-2'>{product.category}</span>
                    ...
                    <div className='p-4 flex flex-col flex-grow'>
                        ...
                        <button className='bg-yellow-400 shadow-sm hover:bg-yellow-500 text-black font-semibold px-4 py-2 text-sm rounded-lg w-full mt-auto'>Add To Cart</button>
                    </div>
                </div>
            ))}
        </section>
